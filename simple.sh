...
#          /opt/jobsub_lite/bin/jobsub_submit -G icarus -N 1 --memory=1800MB --disk=25GB --expected-lifetime=1h --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC,OFFSITE --append_condor_requirements=(TARGET.HAS_Singularity==true) --singularity-image=/cvmfs/singularity.opensciencegrid.org/fermilab/fnal-wn-sl7:latest file:///exp/icarus/app/users/aanilkum/ICARUS-NuMI-CC0pi-Selection/batch/submit.sh -- --project=/pnfs/icarus/scratch/users/aanilkum/1muNp0pi_uncontained_project
...
export BEARER_TOKEN_FILE=$PWD/.condor_creds/icarus_1b78db904e.use
#export BEARER_TOKEN_FILE=$PWD/.condor_creds/icarus.use
...
CVMFS_REPO_LIST=(icarus larsoft sbn)
...
redirect_output_finish(){
    ...
    IFDH_CP_MAXRETRIES=1 ${JSB_TMP}/ifdh.sh cp ${JSB_TMP}/JOBSUB_ERR_FILE.truncated https://fndcadoor.fnal.gov:2880/fermigrid/jobsub/jobs/2026_05_08/c3ad61c3-d2d1-419e-aaf4-679be3884eb8/submit.sh2026_05_08_112739c3ad61c3-d2d1-419e-aaf4-679be3884eb8cluster.$CLUSTER.$PROCESS.err
    IFDH_CP_MAXRETRIES=1 ${JSB_TMP}/ifdh.sh cp ${JSB_TMP}/JOBSUB_LOG_FILE.truncated https://fndcadoor.fnal.gov:2880/fermigrid/jobsub/jobs/2026_05_08/c3ad61c3-d2d1-419e-aaf4-679be3884eb8/submit.sh2026_05_08_112739c3ad61c3-d2d1-419e-aaf4-679be3884eb8cluster.$CLUSTER.$PROCESS.out
...
export JOBSUB_EXE_SCRIPT=$(ls submit.sh 2>/dev/null)
if [ "$JOBSUB_EXE_SCRIPT" = "" ]; then
     export JOBSUB_EXE_SCRIPT=$(find . -name submit.sh -print | head -1)
fi
chmod +x $JOBSUB_EXE_SCRIPT
${JSB_TMP}/ifdh.sh log "$USER:$JOBSUBJOBID BEGIN EXECUTION $JOBSUB_EXE_SCRIPT   --project=/pnfs/icarus/scratch/users/aanilkum/1muNp0pi_uncontained_project "
...
echo `date` $JOBSUBJOBID BEGIN EXECUTION $JOBSUB_EXE_SCRIPT --project=/pnfs/icarus/scratch/users/aanilkum/1muNp0pi_uncontained_project >&2
echo `date` $JOBSUBJOBID BEGIN EXECUTION $JOBSUB_EXE_SCRIPT --project=/pnfs/icarus/scratch/users/aanilkum/1muNp0pi_uncontained_project

 $JOBSUB_EXE_SCRIPT --project=/pnfs/icarus/scratch/users/aanilkum/1muNp0pi_uncontained_project 
JOB_RET_STATUS=$?
...
${JSB_TMP}/ifdh.sh log "$JOBSUBJOBID aanilkum:submit.sh COMPLETED with return code $JOB_RET_STATUS"
